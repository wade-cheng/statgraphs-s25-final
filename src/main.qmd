...
```{r}
bb_train_edit_3 <- baseball |>
  select(inning, is_home_run, outs_when_up) |>
  filter(!is.na(inning), !is.na(is_home_run), !is.na(outs_when_up),
         !(inning == "10"), !(inning == "11"),
         !(inning == "12"), !(inning == "13"))

bb_train_edit_3 |>
  ggplot(aes(x = inning, y = outs_when_up)) +
  geom_point(aes(color = is_home_run), alpha = 0.5, size = 5)# +
  #scale_color_gradient(low = "white", high = "black") +
  #scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  #scale_y_continuous(breaks = c(0, 1, 2))
bb_train_edit_3 |>
  ggplot(aes(x = inning, y = outs_when_up)) +
  geom_point(aes(color = is_home_run), alpha = 0.5) +
  geom_count() #+
  #facet_grid(~ is_home_run) +
  #scale_color_gradient(low = "white", high = "black") +
  #scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 7, 8, 9)) +
  #scale_y_continuous(breaks = c(0, 1, 2))
bb_train_edit_3 |>
